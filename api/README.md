...
| `group_by` | 쏠림 방지 그룹 기준: `station`(기본) / `dong` / `gu` |
| `max_per_group` | 한 그룹 최대 노출 수 (기본 0 = 제한 없음) |
...
`rtms_연립다세대_전월세_서울_202608.csv` — 국토교통부 실거래가(RTMS) 연립다세대 전월세, 2026년 8월.
**8,885건 → 중복 제거 후 8,348건**, 서울 **25개 자치구 · 296개 법정동** 전역.

`LISTINGS_CSV` 환경변수로 다른 파일을 물릴 수 있다
(이전에 쓰던 19개 동네 큐레이션본 `rtms_같이살집_최종_202608.csv` 포함).

> 서울 전역본에는 **`동네`·`위도`·`경도` 컬럼이 없다.** 주소만 있으므로 좌표는 전적으로
> 지오코딩 캐시에서 오고, 캐시에 없는 주소는 매물 목록에서 제외된다
> (`/api/health`의 `skipped_no_coords`로 확인).

### 지역 구분

원본에는 `시군구`("서울특별시 송파구 문정동")뿐이라 자치구와 법정동을 파싱해 `gu`·`dong`에 담는다.
결과 쏠림을 막는 그룹 기준은 `group_by`로 고른다.

| 값 | 개수 | 성격 |
|---|---|---|
| `station` (기본) | ~75 | 최근접 역. 통근이 축인 서비스라 가장 자연스럽다 |
| `dong` | 296 | 법정동. 잘아서 화곡동(372건) 같은 곳에 몰린다 |
| `gu` | 25 | 자치구. 굵어서 구 안의 통근 편차가 묻힌다 |

최근접 역은 기동 시 매물별로 한 번만 계산해 캐시한다.
